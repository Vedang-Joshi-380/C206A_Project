...
def bezier_curve(p0, p1, p2, p3, t):
    ...
    term = (1 - t)
    return (term**3) * p0 + 3*t*(term**2) * p1 + 3*term*(t**2)*p2 + (t**3)*p3

def generate_bezier_waypoints(x1, y1, theta1, x2, y2, theta2, offset=1.0, num_points=100):
...
def plan_curved_trajectory(target_position):
    ...
    waypoints = generate_bezier_waypoints(x1, y1, yaw, x2, y2, yaw, offset=0.02, num_points=100)
...
if __name__ == '__main__':
    ...
    trajectory = plan_curved_trajectory((5.0,5.0))

    # For testing
    #trajectory = generate_bezier_waypoints(0.0, 0.0, np.pi/2, 0.2, 0.2, np.pi/2, offset=0.1, num_points=100)
